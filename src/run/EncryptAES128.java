...
public class EncryptAES128{
    ...
    private static int[][] mixColumns(int[][] state){
        int[][] result = new int[4][4];
        for(int i = 0; i < 4; i++){
            result[0][i] = (multiplyBy2(state[0][i]) ^ multiplyBy3(state[1][i]) ^ state[2][i] ^ state[3][i]);
            result[1][i] = (state[0][i] ^ multiplyBy2(state[1][i]) ^ multiplyBy3(state[2][i]) ^ state[3][i]);
            result[2][i] = (state[0][i] ^ state[1][i] ^ multiplyBy2(state[2][i]) ^ multiplyBy3(state[3][i]));
            result[3][i] = (multiplyBy3(state[0][i]) ^ state[1][i] ^ state[2][i] ^ multiplyBy2(state[3][i]));
        }
        return result;
    }

    private static int multiplyBy2(int num){
        if(num < 0x80) return (num << 1);
        else return ((num << 1) ^ 0x1b);
    }

    private static int multiplyBy3(int num){
        return (multiplyBy2(num) ^ num);
    }


    public String encryptAES128(String inputs, String key){
    	inputs = Base64.getEncoder().encodeToString(inputs.getBytes());
        ...
        int chunkSize = 16;
        for(int q = 0; q < in; q += chunkSize){
        	int end = Math.min(in, q + chunkSize);
